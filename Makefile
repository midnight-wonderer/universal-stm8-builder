...
INCLUDE_PATHS += $(shell (find $(VENDOR_DIR) -maxdepth 3 -type f -name '*.h' -exec dirname {} \; 2>/dev/null) | sort | uniq)
...
VENDOR_SOURCE_FILES += $(shell (find $(VENDOR_DIR) -maxdepth 3 -type f -name '*.c' 2>/dev/null))
...
LDFLAGS = -lstm8 --out-fmt-ihx $(CFLAGS)
